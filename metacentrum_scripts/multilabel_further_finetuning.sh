...
venv/bin/python3 src/multilabel.py --epochs 1 --batch-size 64 --learning-rate 1e-5 --warmup-ratio 0.05 --model-path further_multilabel_metacentrum_output existing long_multilabel_output
mkdir results
venv/bin/python3 src/flores_evaluation.py --model-path further_multilabel_metacentrum_output --type multilabel --encoder-path trainer_output/multilabel_encoder.pkl --output "results/flores_further_multilabel_long.txt"
